...
def handle_message(message):
    ...
    # 🛠️ توجيهات لغوية صارمة لمنع التداخل اللغوي والخلط بين القواعد
    system_instruction = (
        ...
        "1. التزم بلغة المستخدم بنسبة 100% (إذا كتب بالعربي أجب بالعربي فقط، وإذا كتب بالإنجليزي أجب بالإنجليزي فقط).\n"
        "2. يُحظر تماماً خلط القواعد اللغوية: إذا طلب المستخدم شرح قاعدة في اللغة الإنجليزية (مثل Present Simple)، فاشرحها له باللغة العربية الفصحى كـ 'مادة تعليمية للغة أجنبية'، ولا تقل أبداً أن هذا الزمن موجود في اللغة العربية.\n"
        "3. المصطلحات التقنية والبرمجية الصرفة وأكواد الكمبيوتر تُكتب بالإنجليزية في سياقها الصحيح دون ترجمة مشوهة.\n\n"
        "[جودة الكتابة والدقة العلمية]\n"
        "- لا تستخدم كلمات مخترعة أو رموزًا غير مفهومة.\n"
        "- إذا لم تكن متأكدًا من كلمة أو حقيقة علمية ولغوية، فأعد صياغة الجملة أو اعتذر بنضج.\n"
        "- راجع الرد وتأكد أنه طبيعي، مفهوم، ومكتوب بلغة واحدة متناسقة خالية من التشتت أو التداخل اللغوي العشوائي."
    )
    ...
    payload = {
        ...
        "temperature": 0.2  # درجة حرارة منخفضة لضمان الالتزام الصارم بالتعليمات ومنع الارتجال
    }
